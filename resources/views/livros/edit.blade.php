          ...
          <form method="POST" action="/livros/create" enctype="multipart/form-data" > 
            <div class="col-md-12" >
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="sinopse" name="sinopse" value="{{ $livro->sinopse }}">
              </div>
              <div class="form-group">
                <label for="anodepublicacao">Ano de Publicação</label>
                <input type="date" class="form-control" id="anodepublicacao" name="anodepublicacao" value="{{ $livro->anopublicacao }}">
              </div>
              <div class="form-group" >
              ...
              @endforeach
                <?php $autoresLivro = array(); ?>
                @foreach($livro->autores as $autor)
                      <?php array_push($autoresLivro, $autor->id); ?>
                @endforeach       
                <div class="form-group">
                    ...
                    <select multiple="multiple" size="5" id="autor" name="autor[]" style="width: 100%; ">
                      @foreach($autores as $autor)
                        ...
                        @if(in_array("".$autor['id']."",$autoresLivro ) )
                          <option value="{{ $autor['id'] }}" selected> {{ $autor['nome'] }} {{ $autor['sobrenome'] }} </option>
                        @else
